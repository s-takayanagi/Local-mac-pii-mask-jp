...
| メイン画面 | ファイル別の検出結果 |
|---|---|
| ![メイン画面](docs/images/top1.png) | ![ファイル別検出結果](docs/images/top2.png) |

| 置換ログ（サマリー） | 置換ログ（詳細） |
|---|---|
| ![置換ログ サマリー](docs/images/top3.png) | ![置換ログ 詳細](docs/images/top4.png) |

---

> ## ⚠️ 重要なお知らせ（ご利用前に必ずお読みください）
>
> - **本プロジェクトは技術プレビュー版です。** 仕様・構成・タグ名・出力形式は予告なく変更される可能性があります。業務での本番利用は自己責任でお願いします。
> - **ライセンスは現在検討中です。** 本リポジトリに含まれるコードの再配布・商用利用・改変は、正式なライセンス決定までお控えください（依存ライブラリ・モデルのライセンスとは別物です）。
> - **日本語文書を主な対象としています。** 形態素解析・NER・プロンプトはすべて日本語前提で設計されており、英語その他の言語は十分に検証していません。
> - **macOS（Apple Silicon / 16GB 以上）での利用を前提としています。** Windows / Linux / Intel Mac での動作確認は行っていません。
> - **MCP サーバー機能は実装済みですが未検証です。** Claude Desktop 連携は設定例として提供しているもので、実運用での動作保証はありません。フィードバック歓迎です。
> - **マスキング結果の 100% の正確性は保証されません。** LLM ベースの検出は見落とし・過検出が発生しえます。**外部共有前に必ず人間の目で最終確認してください。**
> - **本ツールは無保証（AS-IS）で提供されます。** 利用に伴うあらゆる損害について、作者・コントリビュータは責任を負いません。
> - **Python / GiNZA バージョンは固定方針です。** 本プロジェクトは `ja-ginza-electra` → `transformers` → `tokenizers 0.13.3` の依存連鎖により、**Python 3.11 のみサポート** します（3.12 は `tokenizers` の wheel が配布されておらず、ソースビルドも新しい Rust で失敗するため対象外）。`spaCy` / `GiNZA` / `ja-ginza-electra` のメジャー更新、および Docker ベースイメージの Python 3.12+ への更新は **当面見送り**、Dependabot でも無視する設定としています。新しい Python / GiNZA への追従は、上流の対応状況を見て手動で計画的に実施します。
> - **バグ報告・フィードバックは GitHub Issues** までお願いします。
...
> ⚠️ MCP サーバー機能は実装済みですが、現時点で十分な動作検証を行っていません。以下は設定例として参考にしてください。
...
| ファイル | 内容 |
|---|---|
| [LICENSE](LICENSE) | 本体コードのライセンス（暫定・検討中） |
| [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md) | 依存ライブラリ・LLM モデルのライセンス一覧 |
| [DISCLAIMER.md](DISCLAIMER.md) | 免責事項（技術プレビュー版としての注意事項） |
...
**検討中（TBD）** — 本プロジェクトは技術プレビュー版であり、本体コードのライセンスは現在検討中です。正式決定までは、再配布・商用利用・改変はお控えください。参照・個人検証目的での利用は歓迎します。詳細は [LICENSE](LICENSE) を参照してください。

なお、下記の依存コンポーネント・モデルには各自のライセンスが適用されます（詳細は [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md)）。本体のライセンスが決定しても、これらの条件は引き続き遵守が必要です。
