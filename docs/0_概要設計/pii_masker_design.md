...
**作成日**: 2026-04-18  
**ステータス**: 実装完了（本ドキュメントは設計当時の意思決定を記録したものです。最新の仕様は各詳細設計ドキュメントを参照してください）
...
### 3.1 LFM2-350M-PII-Extract-JP の評価と採用状況

当初 Layer3（LLMマスキング層）の第一候補として検討していたが、
ライセンス上の懸念から一旦見送りとした。その後、LFM2 専用モードとして実装済み。
...
将来的に LFM2 採用に切り替えられる設計にした。

> **実装済み（2026-04-19）**: LFM2-350M-PII-Extract-JP の専用パーサーを `core/layer3_llm.py`
> に実装済み。LM Studio でモデル名に "lfm2" を含むモデルを選択すると自動で専用モードに切り替わる。
> ライセンスの商用利用要件（売上 $10M 以上の企業は `sales@liquid.ai` に連絡）を確認の上で使用すること。
>
> **動作確認済みモデル（2026-04-19）**: `google/gemma-3n-e4b`（**推奨**）/ `qwen/qwen3-1.7b` / `LFM2-350M-PII-Extract-JP` で手動テスト済み。

### 3.2 採用コンポーネントのライセンス確認済み一覧

| コンポーネント | ライセンス | 商用利用 | 備考 |
|---|---|---|---|
| **Gemma 3n E4B** (Masker/Reviewer) | Gemma Terms of Use | ✅ 商用利用可 | **推奨モデル**・動作確認済み |
| **Qwen3 1.7B** (Masker/Reviewer) | Apache 2.0 | ✅ 無制限 | 動作確認済み・軽量 |
...
| メールアドレス | `[メール]` | `[\w.+-]+@[\w-]+\.(?:[a-zA-Z]{2,}\.)*[a-zA-Z]{2,}` |
| 郵便番号 | `[郵便番号]` | `\d{3}[-−]\d{4}` |
...
# LFM2-350M-PII-Extract-JP 専用システムプロンプト
...
入力フォーマット: 常に【原文】と【マスク済みテキスト】の2セクションで渡されます。
...
def _call_lm_studio(system: str, user: str, url: str, model: str, role: str,
                    timeout: int = 120, temperature: float = 0.05) -> dict | None:
    # ...（エラーハンドリング・ログ含む実装は core/layer3_llm.py 参照）
    pass

def call_masker(text: str, model: str, url: str,
                excluded_tags: set[str] | None = None) -> dict | None: ...

def call_reviewer(masked_text: str, model: str, url: str,
                  excluded_tags: set[str] | None = None,
                  original_text: str | None = None) -> dict | None: ...
```
...
def mask_text(
    ...
    model: str,
    lm_studio_url: str,
    enabled_layers: set[str] | None = None,   # デフォルト: {"layer1","layer2","layer3","layer4"}
    excluded_tags: set[str] | None = None,    # 除外タグ例: {"[氏名]", "[電話番号]"}
) -> MaskResult:
    # Layer1〜4 を順に適用。enabled_layers で各層をON/OFF可能。
    # Layer4 Reviewer は call_reviewer(masked, ..., original_text=text) で原文も渡す。
    # 各層失敗時は手前の層の結果でフォールバック。
    # 戻り値の MaskResult.replacements には各 dict に "layer" キーが付与される。
    ...
```
...
16. ~~LFM2-350M-PII-Extract-JP への差し替え（ライセンス確認後）~~ → **実装完了**（2026-04-19）
...
LM Studio で LFM2-350M-PII-Extract-JP をロードして選択するだけで自動的に専用モードになります。
コードの変更は不要です。`_is_lfm2_model(model)` 関数がモデル名に "lfm2" を含む場合に True を返し、
`call_masker()` / `call_reviewer()` 内でパスを切り替えます。
