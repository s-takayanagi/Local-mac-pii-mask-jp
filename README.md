...
> - **Python / GiNZA バージョンは固定方針です。** 本プロジェクトは `ja-ginza-electra` の依存制約により **Python 3.11〜3.12** のみサポートします。`spaCy` / `GiNZA` / `ja-ginza-electra` のメジャー更新、および Docker ベースイメージの Python 3.13+ への更新は **当面見送り**、Dependabot でも無視する設定としています。新しい Python / GiNZA への追従は、上流の対応状況を見て手動で計画的に実施します。
