...
class ProcessResult:
    ...
    layer_totals: dict         = field(default_factory=dict)  # {"layer1": N, ...}
    replacements_log: list[dict] = field(default_factory=list)  # {"location", "original", "tag", "layer"}
    warnings: list[str]        = field(default_factory=list)  # docx のみ生成
```
...
**警告機能（`warnings`）**
- テーブル内の先頭セルが「氏名」「ふりがな」などのラベルと一致し、
  隣の値セルが未マスクの場合に `ProcessResult.warnings` に警告メッセージを追加
- xlsx / pptx ハンドラーでは `warnings` は常に空配列
