...
| [LM Studio](https://lmstudio.ai/) | Qwen2.5-7B をローカル推論 | 完全オフライン |
...
1. LM Studio を起動
2. `Qwen2.5-7B-Instruct`（Q4_K_M、約5GB）をダウンロード
...
| `LM_STUDIO_MODEL` | `qwen/qwen3.5-9b` | 使用するモデル名（LM Studio のモデル ID と一致させる） |
...
| コンポーネント | ライセンス | 備考 |
|---|---|---|
| Qwen2.5-7B-Instruct / 各汎用 LLM | Apache 2.0 等 | Layer3/4 の推奨モデル。LM Studio で任意モデルを使用可 |
| LFM2-350M-PII-Extract-JP | LFM Open License v1.0 | 特化型モードで対応済み。商用利用は売上 $10M 以上の場合は要ライセンス確認 |
